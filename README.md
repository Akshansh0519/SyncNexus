...
| **Vector Ingestion & RAG Fallback** | Crashes on `.add()` or when Chroma container is waking up (`<!DOCTYPE html`) | **ChromaDB `.upsert()` with `LocalVectorStore` in-memory fallback** guarantees vector database ingestion and query retries never crash if cloud vector nodes are warming up |
| **Google Gemini SDK** | Deprecated `@google/generative-ai` v1 (`getGenerativeModel` errors) | **`@google/genai` v2 SDK** (`ai.models.embedContent` and `ai.models.generateContent`) with structured markdown AI Chatbot output formatting |
...
| **UI & Avatar Distinction** | Generic Tailwind / Bootstrap templates with random overlapping avatar colors | **Modern Swiss Brutalism + Cyberpunk Glassmorphism** with smart case-insensitive user hashing (`T1` vs `T2` unique high-contrast gradient circles) and graceful Render ~50s free tier spin-up notices |
...
| `POST` | `/api/rooms/:id/files/upload` | Bearer | Single-step server-proxied file upload (bypasses browser CORS for cloud storage) |
...
| **Direct & Server-Proxied S3 Uploads** | Supports presigned direct-to-MinIO uploads for local efficiency, alongside a single-step `POST /files/upload` proxy route to cleanly bypass browser CORS when connecting to cloud object stores (Filebase IPFS/S3). |
...
| **ChromaDB Upsert & LocalVectorStore Fallback** | Replaced `.add()` with `.upsert()` and added an in-memory `LocalVectorStore` fallback. If an ingestion worker runs while Chroma instances are offline/waking up on cloud free tiers, the pipeline degrades gracefully instead of crashing with HTML parse errors. |
...
| **Early Returns for RAG & @google/genai v2** | Uses modern `@google/genai` (`ai.models.embedContent` and `generateContent`) with bulleted Markdown formatting and strict early checks before embedding/inference, protecting usage quotas. |
